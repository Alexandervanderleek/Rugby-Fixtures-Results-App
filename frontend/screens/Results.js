...
export default function Results() {
  ...
  // const adUnitId =  'ca-app-pub-4487344159300856/3414417935' ;
  // const adUnitId2 =  'ca-app-pub-4487344159300856/9245329115' ;
  ...
  if(!isLoading){
    ...
    {((data["league-"+pickerRef?.current?.props?.selectedValue])?.filter((item)=>{return item.status === "FT"}).length === 0 && !isLoadingSmall) && (
       <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
        <Text style={styles.ErrorText}>
          No Past Results
        </Text>
      ...
      {((data["league-"+pickerRef?.current?.props?.selectedValue])?.filter((item)=>{return item.status === "FT"}).length > 0 && !isLoadingSmall) && (
        ...
        <FlatList style={{height: '100%' }}
          data={(data["league-"+pickerRef.current.props.selectedValue]).filter((item)=>{return item.status === "FT"}).reverse()}
          renderItem={({item})=> (
